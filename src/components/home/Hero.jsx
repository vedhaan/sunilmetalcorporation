  ...
  return (
    <section className="relative bg-cream overflow-hidden">
      <motion.div
        ...
        animate="visible"
        variants={container}
        className="max-w-6xl mx-auto px-6 py-24 md:py-32 relative z-10 grid lg:grid-cols-2 gap-16 items-center"
      >
        <div>
          ...
          <motion.h1 variants={item} className="font-display text-4xl md:text-6xl font-bold text-ink leading-[1.05] tracking-tight">
            ...
            <span className="text-bronze-deep underline decoration-oxide decoration-4 underline-offset-4">
              Since 1985
            </span>
          </motion.h1>
          ...
          <motion.div variants={item} className="mt-10 flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <MotionLink
              ...
              whileTap={{ scale: 0.97 }}
              className="bg-oxide hover:bg-bronze-deep text-cream font-semibold px-8 py-3 rounded transition-colors"
            >
              Request a Quote
            </MotionLink>
            <motion.a
              ...
              whileHover={{ scale: 1.03 }}
              whileTap={{ scale: 0.97 }}
              className="border border-ink/30 text-ink hover:bg-ink/5 font-semibold px-8 py-3 rounded transition-colors"
            >
              Chat on WhatsApp
            </motion.a>
          </motion.div>

          <motion.div variants={item} className="mt-10 flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-xs uppercase tracking-wide text-bronze-deep">
            <span>MRAI Member</span>
            <span className="text-bronze-core">•</span>
        ...
        <div className="hidden lg:block relative w-full aspect-square max-w-md mx-auto">
          <svg viewBox="0 0 320 320" className="absolute inset-0 w-full h-full">
            <defs>
              <marker id="flowArrow" markerWidth="7" markerHeight="7" refX="3.5" refY="3.5" orient="auto-start-reverse">
                <path d="M0,0 L7,3.5 L0,7 Z" fill="#9C6B2E" />
              </marker>
            </defs>
            {[
            ...
            <motion.path
              ...
              fill="none" stroke="#6E4419" strokeWidth="2" markerEnd="url(#flowArrow)"
              initial={{ pathLength: 0, opacity: 0 }}
              animate={{ pathLength: 1, opacity: 1 }}
          ...
          <motion.p
            ...
            className="absolute top-1/2 left-1/2 -translate-x-1/2 translate-y-9 text-center font-mono text-[9px] uppercase tracking-wide text-bronze-deep w-28"
          >
          ...
          {steps.map((s, i) => (
            <motion.div
              ...
              className="absolute -translate-x-1/2 -translate-y-1/2 bg-cream border border-bronze-core/50 rounded-lg shadow-sm px-3 py-2 w-32 text-center"
            >
              <p className="font-mono text-[10px] text-bronze-core">{s.n}</p>
